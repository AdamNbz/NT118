...
const OrderDetailPage = () => {
  ...
  const fetchDetail = async () => {
    try {
      setLoading(true);
      const detail = await orderApi.getOrderDetail(Number(id));
      setData(detail);
    } catch (error) {
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.statusSection}>
          ...
          <MaterialCommunityIcons 
            name={order.status === 'delivered' ? 'package-variant-closed' : 'truck-delivery-outline'} 
            size={40} 
            color="white" 
          />
        </View>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.addressText}>ID Địa chỉ: {order.shippingAddressId}</Text>
          <Text style={styles.addressSub}>Dữ liệu địa chỉ chi tiết sẽ được hiển thị khi tích hợp API địa chỉ đầy đủ.</Text>
        </View>
        ...
        {order.status === 'pending' && (
          <TouchableOpacity 
            ...
            disabled={cancelling}
          >
            {cancelling ? <ActivityIndicator color="#ef4444" /> : <Text style={styles.cancelButtonText}>Hủy đơn hàng</Text>}
          </TouchableOpacity>
        )}
      </ScrollView>
    </SafeAreaView>
